...
var VisCellCreator = {
    ...
    methods: {
        ...
        dropdownMenuClick(event) {
            let el_ = document.getElementsByClassName("el-dropdown-link")[0]
            let fileName = event.currentTarget.innerText
            el_.textContent = fileName

            vm.selectedFileName = fileName.split(".")[0]

            let el = document.createElement("div")
            el.setAttribute("id", "test-div")
            let thisAddCells = document.getElementById('table-cell-creator')
            const tbcProps = {
                index: vm.id,
                sbIndex: vm.sbId,
                layout: 'single-column',
                tableData: vm.selectedFileName,
                startNewLine: false,
            }
            let referenceNode = thisAddCells
            referenceNode.parentNode.insertBefore(el, referenceNode.nextSibling);

            const newTC = createApp(SpreadsheetCell, tbcProps)
            newTC.mount('#test-div')
            let cnt = $("#test-div").contents();
            $("#test-div").replaceWith(cnt);
...
<template>
    <el-row id="table-cell-creator">
        <el-col :span="24">
            <div class="vis-cell-picker">
                <div class="pick-vis-type">
                    <el-col>
                        <div class="text center">Pick from Uploaded CSV Files</div>
